...
float win_round = 0.0;
bool rep_note = true;
Hero COM = { "Com", 3, rec, 1, 5, 0, 0, 0.0, 0 },
MY = { "Love", 3, rec, 0, 5, 0, 0, 0.0, 0 };
//对电脑、玩家档案初始定义
//定义局数、回合数、玩家胜利局数
...
void win_rate(Hero& COM_WIN, Hero MY_WIN) //计算成员胜率
{
	...
	if (Whole > 1)
	{
		MY_WIN.win_rate = win_round / (Whole - 1) * 100;
		COM_WIN.win_rate = (Whole - win_round - 1) / (Whole - 1) * 100;
	}
}
...
void note(Hero& NOTE) //在旁输出特殊情况
{
	...
	if (NOTE.act == pra)
	{
		cout << NOTE.name << "进行了虔诚的\'祈 祷\',生命增加1,暴击率增加" << NOTE.crit_plus << "\%!" << endl;
	}

	if (NOTE.act == rep)
	{
		cout << NOTE.name << "进行了虔诚的\'忏 悔\',生命增加1,暴击率增加" << NOTE.crit_plus << "\%!" << endl;
	}

	if (NOTE.pra_time == 3 && rep_note)
	{
